...
import './ProductList.css' // Import CSS for layout and styles
import CartItem from './CartItem'; // Imported but not rendered directly here

function ProductList({ onHomeClick, onOpenCart }) { 
...
    ];
    
    // Inline styles for Navbar (Sticky and Blue)
    const styleObj = {
        backgroundColor: 'rgb(70 120 163)',
        color: '#fff!important',
        padding: '15px',
        display: 'flex',
...
        e.preventDefault();
        // Calls handleOpenCartModal from App.jsx
        onOpenCart(); 
    };

    const handlePlantsClick = (e) => {
        e.preventDefault();
        alert('About Us functionality pending');
    };

    // Function to handle category menu click (smooth scroll)
    const handleCategoryClick = (e, categoryId) => {
        e.preventDefault();
        const element = document.getElementById(categoryId);
        
        if (element) {
            // Target the scrollable container (modal-content)
            const modalContent = document.querySelector('.modal-content');
            const headerHeight = document.querySelector('.product-sticky-header').offsetHeight;
            const elementPosition = element.offsetTop;
            
            // FIX: Scroll the modal content container
    ...
    return (
        <div className="product-list-wrapper">
            ...
            <div className="product-sticky-header">
                ...
                {/* MAIN NAV BAR (Top Blue Bar) */}
                <div className="navbar product-navbar-main" style={styleObj}>
                    <div className="tag">
                        <div className="luxury">
                            <img src="https://cdn.pixabay.com/photo/2020/08/05/13/12/eco-5465432_1280.png" alt="" />
                            {/* Use onHomeClick prop to go back to the main home screen */}
                    ...
                    <div className="nav-right-container" style={styleObjUl}>
                        ...
                        {/* The modal close button will be positioned here via CSS */}
                    </div>
                </div>
                    ...
                    <div className="submenu-links">
                        {plantsArray.map((category) => (
                            <a 
                                ...
                                // Create anchor ID by replacing spaces with hyphens
                                href={`#${category.category.replace(/\s/g, '-')}`}
                                onClick={(e) => handleCategoryClick(e, category.category.replace(/\s/g, '-'))}
            ...
            </div> {/* END product-sticky-header */}

            {/* PRODUCT GRID CONTENT */}
            <div className="product-grid">
                {plantsArray.map((category, index) => ( 
                    /* Add ID to target this section for scrolling */
                    <div key={index} className="category-section" id={category.category.replace(/\s/g, '-')}> 
                        ...
                        <div className="product-list">
                            {category.plants.map((plant, plantIndex) => {
                                const added = isProductInCart(plant.name);
                                return (
                                ...
                                <button
                                    ...
                                    disabled={added} 
                                    style={{ backgroundColor: added ? '#ccc' : 'rgb(70 120 163)' }}
                                >
